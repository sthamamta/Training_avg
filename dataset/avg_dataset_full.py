...
class MRIDatasetFull(Dataset):
    ...
    def __getitem__(self, index):
        ...
        first_hr_idx = hr_index-1
        last_hr_idx = hr_index+1


        if self.axis == 0:
            lr_image = self.lr_array[lr_index,:,:]
            hr_image = self.hr_array[hr_index,:,:]
            first_hr_image = self.hr_array[first_hr_idx,:,:]
            last_hr_image = self.hr_array[last_hr_idx,:,:]
        elif self.axis == 2:
            lr_image = self.lr_array[:,:,lr_index]
            hr_image = self.hr_array[:,:,hr_index]
            first_hr_image = self.hr_array[:,:,first_hr_idx]
        ...
        first_hr_tensor = min_max_normalize(first_hr_image)
        last_hr_tensor = min_max_normalize(last_hr_image)

        

        lr_tensor = torch.from_numpy(lr_tensor).float()
        hr_tensor = torch.from_numpy(hr_tensor).float()
        first_hr_tensor = torch.from_numpy(first_hr_tensor).float()
        last_hr_tensor = torch.from_numpy(last_hr_tensor).float()
        ...
        last_hr_tensor = torch.unsqueeze(last_hr_tensor,0)

        return {
                "lr": lr_tensor, 
                "first_hr": first_hr_tensor,
                "second_hr":hr_tensor,
                "third_hr":last_hr_tensor
        }