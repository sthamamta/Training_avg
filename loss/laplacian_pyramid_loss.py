...
def gaussian_kernel(size=5, device=torch.device('cpu'), channels=1, sigma=1, dtype=torch.float):
...
def create_laplacian_pyramid(x, kernel, levels):
    upsample = torch.nn.Upsample(scale_factor=2,mode='bicubic') # Default mode is nearest: [[1 2],[3 4]] -> [[1 1 2 2],[3 3 4 4]]
    downsample = torch.nn.Upsample(scale_factor=0.5,mode='bicubic') # Default mode is nearest: [[1 2],[3 4]] -> [[1 1 2 2],[3 3 4 4]]
    pyramids = []
    current_x = x
...
class LaplacianPyramidLoss(torch.nn.Module):
    def __init__(self, max_levels= 4, channels=1, kernel_size=9, sigma=1.5, device=torch.device('cuda'), dtype=torch.float):
...
if __name__ == '__main__':
    ...
    output = torch.rand(1,1,100,100)
    loss = LaplacianPyramidLoss()
    loss_value =  loss(input, output)